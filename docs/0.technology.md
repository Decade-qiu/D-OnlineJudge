...
### 亮点五：WebSocket 实时消息推送

为了在异步判题的架构下，向用户提供实时的结果反馈，我们采用了 **WebSocket** 技术，取代了传统的客户端轮询方案。
...
- **实时性与高效性**: 一旦判题完成，结果会立即通过已建立的长连接推送给前端，延迟极低。相比于轮询方案，它避免了大量的无效 HTTP 请求，极大地节省了服务器和网络资源。
- **体验连贯**: 用户无需停留在当前页面或手动刷新，即可收到判题完成的通知，交互体验非常流畅。
- **实现**: 后端通过标准的 `javax.websocket` API (`@ServerEndpoint`) 创建 WebSocket 服务；前端通过原生的 `WebSocket` API 进行连接和监听。
